...
namespace BlackCat\Database\Packages\MagicLinks\Service;
 ...
 */
final class MagicLinksAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private MagicLinkRepository $magicLinkRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'magic_links' => $this->magicLinkRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\MagicLinks\ModuleException("Record {$idStr} not found for locking.");
            }
