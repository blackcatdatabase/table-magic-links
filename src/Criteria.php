...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'fingerprint', 'subject', 'user_id', 'context', 'expires_at', 'created_at' ];
    }
    ...
    public static function fromDb(
        ...
        if (
            $tenantId !== null
            && $tenantColumn !== ''
            && \method_exists(\BlackCat\Database\Packages\MagicLinks\Definitions::class, 'hasTenant')
            && \BlackCat\Database\Packages\MagicLinks\Definitions::hasTenant()
        ) {
            $c->tenant($tenantId, $tenantColumn);
        }

        if (\method_exists(\BlackCat\Database\Packages\MagicLinks\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\MagicLinks\Definitions::softDeleteColumn();
